...
pub struct Exchange {
    pub id: uuid::Uuid,
    // Since query is a trait object, it needs to be boxed
    // by Boxing, you're allocating the memory on the heap, and then storing the pointer to that memory on the stack.
    // this way the size of the object is known at compile time
                                // and you also communicate to the the pointer can point to any type that implements the trait
...
impl Exchange {
    pub fn new(id: uuid::Uuid, query: &str) -> Self {
        Self {
            id,
