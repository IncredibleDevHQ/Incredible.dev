...
use crate::ast::language_support::{NameSpaceMethods, TSLanguageConfig, ALL_LANGUAGES};
use crate::ast::{symbol::Symbol, text_range::TextRange};
    ...
    pub fn symbols_metadata(
        ...
        self.graph
            ...
            .filter_map(|idx| {
                ...
                match node {
                    NodeKind::Def(def) => {
                        // use is_top_level to determine if the def is a top-level def.
                        // if it is, then it is a global def.
                        // if it is not, then it is a local def.
                        // return true if the def is a top-level def.
                        ...
                        let node_kind = String::from("def");
                        Some(SymbolMetaData {
                            repo_name: repo_name.clone(),
                            relative_path: relative_path.clone(),
                            language_id: language_id.clone(),
                            is_global: is_global,
                            symbol: def.symbol_id.unwrap().name(namespaces).to_owned(), // FIXME: this should use SymbolId::name
                            symbol_type: std::str::from_utf8(def.name(src)).unwrap().to_owned(),
                            range: def.range,
                            node_kind: node_kind.clone(),
                        })
                    }
                    // NodeKind::Ref(r) => {
                    //     let node_kind = String::from("ref");
                    //     Some(SymbolMetaData {
                    //         repo_name: repo_name.clone(),
                    //         relative_path: relative_path.clone(),
                    //         language_id: language_id.clone(),
                    //         is_global: false,
                    //         symbol: r.symbol_id.unwrap().name(namespaces).to_owned(), // FIXME: this should use SymbolId::name
                    //         symbol_type: std::str::from_utf8(r.name(src)).unwrap().to_owned(),
                    //         range: r.range,
                    //         node_kind: node_kind.clone(),
                    //     })
                    // }
