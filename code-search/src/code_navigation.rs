...
use common::ast::{ast_graph::NodeKind, graph_code_pluck::ContentDocument, text_range::TextRange};
use serde::Serialize;

use crate::snippet::{Snipper, Snippet};
use rayon::prelude::*;
...
impl<'a, 'b> CodeNavigationContext<'a, 'b> {
    ...
    fn is_definition(&self) -> bool {
        match self.source_document().symbol_locations() {
    ...
    fn local_definitions(&self) -> Option<FileSymbols> {
        match self.source_document().symbol_locations() {
            Ok(symbol_locations) => {
                ...
                let node_idx =
                    scope_graph.node_by_range(self.token.start_byte, self.token.end_byte)?;
            ...
            Err(_) => None, // Handle the error appropriately, possibly by returning None.
        }
    }
        ...
        self.non_source_documents()
            ...
            .filter_map(|doc| {
                match doc.symbol_locations() {
                    Ok(graph) => {
                        let scope_graph = graph.scope_graph().unwrap(); // Assuming scope_graph() is now safely unwrapped or otherwise handled
    ...
    fn repo_wide_references(&self) -> Vec<FileSymbols> {
        self.non_source_documents()
            .par_bridge()
            .filter_map(|doc| {
                // Directly handle the result of symbol_locations within the match to keep the scope limited
                match doc.symbol_locations() {
                    Ok(graph) => {
                        // Immediately get the scope_graph from graph to use within this scope
                        let scope_graph = graph.scope_graph().unwrap(); // Ensure that unwrap is safe here or handle error appropriately
                        ...
                        let mut data = scope_graph
                            ...
                            .flat_map(|idx| scope_graph.references(idx)) // Assuming references() is a method defined on the scope_graph or its fields
                            .map(|idx| Occurrence {
                                kind: OccurrenceKind::Reference,
                                range: scope_graph.graph[idx].range(),
                                snippet: to_occurrence(
                                    ...
                                    scope_graph.graph[idx].range(),
                                    self.snipper,
                                ), // Assuming to_occurrence is defined elsewhere and does not have lifetime issues
                            })
                            .collect::<Vec<_>>();

                        data.sort_by_key(|occurrence| occurrence.range.start.byte);

                        // Only return Some if data is not empty
                        if !data.is_empty() {
                            Some(FileSymbols {
                                file: doc.relative_path.to_owned(),
                                repo: doc.repo_ref.parse().unwrap(), // Make sure parse().unwrap() is safe or handle it appropriately
                                data,
                            })
                        } else {
                            None
                    ...
                    Err(_) => None, // Handle error or absence of symbol_locations
                }
            })
            .collect()
    }
