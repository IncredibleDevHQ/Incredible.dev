use anyhow::Result;
use common::ast::{graph_code_pluck::ContentDocument, symbol::Symbol};
...
impl Snipper {
    ...
    pub fn all_for_doc(&self, regex: &str, doc: &ContentDocument) -> Result<Option<SnippedFile>> {
        ...
        let snippets = if self.find_symbols {
            ...
            //
            let mut symbols = doc.symbol_locations()?.list(); //TODO: Fix for list
...
mod tests {
    ...
    /// indices.
    fn with_line_ends(s: &str) -> (&str, Vec<u8>) {
        assert!(s.ends_with('\n'));
        let line_ends = s
            .match_indices('\n')
            .map(|(i, _)| i as u8)
